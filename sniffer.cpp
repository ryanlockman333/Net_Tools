...
void* Sniffer::process_rx() {
    ...
    if(!(header->tp_status & TP_STATUS_USER)) { // check if kernel owns it, TP_STATUS_KERNEL
...
void Sniffer::packet_switch(const tpacket2_hdr* const packet, c_uint len) {
    ...
    // Debug, Prints Whol Packet Header Dump Before, TPacket and All
    if(debugFlag) {
    ...
    case ARPHRD_NETROM: { // un-comment to see if real packet or 0's are comming in, 0's mean wire is empty
    ...
    case ARPHRD_IEEE80211_RADIOTAP: { // Fibrechannel
        ...
        case WIFI_MGMT_FRAME: // Management Frame
            ...
            ie80211_Size = sizeof(ieee80211_mgmt_hdr);
            break;
        ...
        case ETH_P_802_2: { // LLC, non DIX type
            ...
            unsigned pSize = len - le16toh(rtap->it_len) - ie80211_Size - sizeof(ieee80211_fcs_hdr);
            ...
            /* encrypted needs to be before data frames to print right **pattern */
                ...
                ofs << "\n\n\n********************802.11 Control Wrapper Frame***********************"
                    << "\n\n802.11 Control Wrapper Frame Time Stamp: " << std::ctime(&t);
                ext = true;
            }
...
void Sniffer::printTPACKET(cuchar_cptr packet, c_uint len) { // pre-pended
    ...
    ofs << "\n~~~TPacket Header(pre-pended)~~~"
        ...
        << (TSTAT == TP_STATUS_KERNEL       ? ": Kernel"
          ...
          : TSTAT == TP_STATUS_COPY         ? ": Copy"
          : TSTAT == TP_STATUS_LOSING       ? ": Losing"
          : TSTAT == TP_STATUS_CSUMNOTREADY ? ": Check Sum Not Ready"
          : TSTAT == TP_STATUS_VLAN_VALID   ? ": Valid VLAN TCI"
          : TSTAT == TP_STATUS_BLK_TMO      ? ": Block Timeout"
          :                                   ": Unknown")
        << "\nLength:       " << tpack->tp_len
...
//  else shouldn't happen
            ...
            for(unsigned oset = 0; oset < len;) {
                ...
                switch((unsigned)ie->ie_gen.id) {
                    ...
                    case WLAN_MGMT_IE_RATES:
                        ...
                        // Print Rates
                        for(std::size_t j = 0; j < (unsigned)ie->ie_gen.len; ++j) {
                            ofs << ((float)(unsigned)((ie->ie_gen.rates[j]) & 0x7F)
                                        * 500.00) / 1000.00 << " Mb/s" // 500 Kbps units
                    ...
                    case WLAN_MGMT_IE_HT_CAPABILITY: // 802.11n
                        ofs << ": HT Capabilities"
                            ...
                            << "\nSpatial Multiplexing PS:  " << (le16toh(ie->ht_capab.capab_info) & 0x000C)
                            << ((~le16toh(ie->ht_capab.capab_info) & 0x000C) == 0x000C 
                             ? ": Static SM Power Save Mode"
                             :    le16toh(ie->ht_capab.capab_info) & 0x0004  == 0x0004
                             ? ": Dynamic SM Power Save Mode"
                             :    le16toh(ie->ht_capab.capab_info) & 0x0008  == 0x0008
                             ? ": Reserved"
                             :    le16toh(ie->ht_capab.capab_info) & 0x000C  == 0x000C
                             ? ": Disabled"
                            ...
                            << "\nRX STBC:                  " << (le16toh(ie->ht_capab.capab_info) & 0x0300)
                            << ((~le16toh(ie->ht_capab.capab_info) & 0x0300) == 0x0300
                             ? ": Disabled"
                             :    le16toh(ie->ht_capab.capab_info) & 0x0100 == 0x0100
                             ? ": One Spatial System"
                             :    le16toh(ie->ht_capab.capab_info) & 0x0200 == 0x0200
                             ? ": One Or Two Spatial Systems"
                             :    le16toh(ie->ht_capab.capab_info) & 0x0300 == 0x0300
                             ? ": One, Two, Or Three Spatial Systems"
                             : ": Unknown RX STBC") // shouldn't happen
                            ...
                            << "\nMax A-MPDU Size:          " << (ie->ht_capab.a_mpdu_param & 0x03)
                            << ((~ie->ht_capab.a_mpdu_param & 0x03) == 0x03
                              ? ": 8K"
                              :   ie->ht_capab.a_mpdu_param & 0x01  == 0x01
                              ? ": 16K"
                              :   ie->ht_capab.a_mpdu_param & 0x02  == 0x02
                              ? ": 36K"
                              :   ie->ht_capab.a_mpdu_param & 0x03  == 0x03
                              ? ": 64K"
                              : ": Unknown Size") // shouldn't happen
                            << "\nMin MPDU Start Spacing:   " << (ie->ht_capab.a_mpdu_param & 0x1C)
                            << ((~ie->ht_capab.a_mpdu_param & 0x1C) == 0x1C
                              ? ": No Restrictions"
                              :   ie->ht_capab.a_mpdu_param & 0x04  == 0x04
                              ? ": 1/4 us"
                              :   ie->ht_capab.a_mpdu_param & 0x08  == 0x08
                              ? ": 1/2 us"
                              :   ie->ht_capab.a_mpdu_param & 0x0C  == 0x0C
                              ? ": 1 us"
                              :   ie->ht_capab.a_mpdu_param & 0x10  == 0x10
                              ? ": 2 us"
                              :   ie->ht_capab.a_mpdu_param & 0x14  == 0x14
                              ? ": 4 us"
                              :   ie->ht_capab.a_mpdu_param & 0x18  == 0x18
                              ? ": 8 us"
                              :   ie->ht_capab.a_mpdu_param & 0x1C  == 0x1C
                              ? ": 16 us"
                              : ": Unknown Start Spacing") // shouldn't happen
                            << "\nMPDU Restricted Bit Sum:  " << (ie->ht_capab.a_mpdu_param & 0xE0);
                            break;
    ...
    // Check To Print HT Control Field (802.11) Through Order Bit
    u32 hctrl = 0;
    if(GetOrder(ie80211_un)) {
        ...
        ofs << "\nLink Adaptation Control:      "
            ...
            << "\nMAI:                      " << (hctrl & 0x0000003C)
            << ((hctrl & 0x0000003C) == 14 ? ": ASELI, MFB/ASELC Interpreted As ASELC"
              :                              ": MCS Request(MRQ), MFB/ASELC Interpreted As MFB");

        // Check MAI To Print MRQ Information
        if(hctrl & 0x0000003C  != 14) {
            ofs << "\nMCS Request(MRQ):          "
                << (hctrl & 0x00000040 ? "1: MCS Feedback" : "0: No MCS Feedback");

            // Check Feedback Bit Set For Sequence Or Reserved Fields Following
            if(hctrl & 0x00000040)
                ofs << "\nMRQ Sequence ID(MSI): " << (hctrl & 0x00000038);
            else
                ofs << "\nReserved Bits(4-6):   " << (hctrl & 0x00000038);
        }

        // Continue Printing HT Control Field
        ofs << "\nMFB Sequence ID(MFSI):    " << (hctrl & 0x000001C0)
            << "\nMFB/ASELC:                " << (hctrl & 0x0000FE00)
            << ((hctrl & 0x0000003C) == 14  ? ": ASELC"
             :  (hctrl & 0x0000FE00) == 127 ? ": MFB: No MCS Feedback Present"
             :  (hctrl & 0x0000FE00) <  127 ? ": MFB: MCS Feedback Present"
             : ""); ///////////////////////////////////////////////////////////////////////////////finish look in book for MFB and ASELC interp
            
        // Finish Printing HT Control Field
        ofs << "\nCalibration Position:     " << (hctrl & 0x00030000)
            << (!(hctrl & 0x00030000) == 0x00000000
             ? ": Not A Calibration Frame"
             :    hctrl & 0x00010000  == 0x00010000
             ? ": Calibration Start"
             :    hctrl & 0x00020000  == 0x00020000
             ? ": Sounding Response"
             :    hctrl & 0x00030000  == 0x00030000
             ? ": Sounding Complete"
             : ": Unknown Calibration Position") // shouldn't happen0
            << "\nCalibration Sequence:     " << (hctrl & 0x000C0000)
            << "\nReserved Bits(21-22):     " << (hctrl & 0x00300000)
            << "\nCSI/Steering:             " << (hctrl & 0x00C00000)
            << (!(hctrl & 0x00C00000) == 0x00C00000
             ? ": No Feedback Required"
             :    hctrl & 0x00400000  == 0x00400000
             ? ": CSI"
             :    hctrl & 0x00800000  == 0x00800000
             ? ": Noncompressed Beamforming"
             :    hctrl & 0x00C00000  == 0x00C00000
             ? ": Compressed Beamforming"
             : ": Unknown CSI/Steering")
            << "\nNDP Announcement:         " << (hctrl & 0x01000000 ? '1' : '0')
            << "\nReserved Bits(26-30):     " << (hctrl & 0x3E000000)
            << "\nAC Constraint:            " << (hctrl & 0x40000000 ? '1' : '0')
            << "\nRDG/More PPDU:            " << (hctrl & 0x80400000 ? '1' : '0');
    }
